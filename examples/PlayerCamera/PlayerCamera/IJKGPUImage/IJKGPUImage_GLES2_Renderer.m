...
IJK_GLES2_Renderer *IJKGPUImage_GLES2_Renderer_create_base(const char *fragment_shader_source)
{
...
fail:
    ...
    if (renderer && renderer->program)
        IJKGPUImage_GLES2_printProgramInfo(renderer->program);
    
    IJKGPUImage_GLES2_Renderer_free(renderer);
    return NULL;
}
...
{
    if (!IJKGPUImage_GLES2_Renderer_isValid(renderer))
        return GL_FALSE;
...
static void IJKGPUImage_GLES2_Renderer_Vertices_reset(IJK_GLES2_Renderer *renderer)
{
...
static void IJKGPUImage_GLES2_Renderer_Vertices_apply(IJK_GLES2_Renderer *renderer)
{
    switch (renderer->gravity) {
        ...
        case IJK_GLES2_GRAVITY_RESIZE:
            IJKGPUImage_GLES2_Renderer_Vertices_reset(renderer);
            return;
        default:
...
static void IJKGPUImage_GLES2_Renderer_Vertices_reloadVertex(IJK_GLES2_Renderer *renderer)
{
...
GLboolean IJKGPUImage_GLES2_Renderer_setGravity(IJK_GLES2_Renderer *renderer, int gravity, GLsizei layer_width, GLsizei layer_height)
{
...
GLboolean IJKGPUImage_GLES2_Renderer_use(IJK_GLES2_Renderer *renderer)
{
    ...
    IJKGPUImage_GLES2_Renderer_Vertices_reset(renderer);
    IJKGPUImage_GLES2_Renderer_Vertices_reloadVertex(renderer);
